...
const app = {
  ...
  render: function () {
    const htmls = this.songs.map((song, index) => {
      return `
        ...
        <div class="watch-list__time">5:00</div>
  ...
  getTotalTimes: function () {
    ...
    audios.forEach((item) => {
      item.onloadedmetadata = function () {
        ...
        const second = Math.floor(item.duration % 60);
        const minute = Math.floor(item.duration / 60);
        const time =
          second < 10 ? minute + ":0" + second : minute + ":" + second;
        item.parentElement.querySelector(".watch-list__time").innerText = time;
      };
    });
  },
  loadCurrentSong: function () {
    ...
    audio.onloadedmetadata = function () {
      ...
      const time = second < 10 ? minute + ":0" + second : minute + ":" + second;
  ...
  handlePlayThumb: function () {
    ...
    for (let j = 0; j < songChoosed.length; j++) {
      if (j !== app.currentIndex) {
        ...
        const pauseThumbBtn__ =
          songChoosed[j].querySelector(".btn-thumb-pause");
  ...
  handleEvents: function () {
    ...
    const defaultPlayBtn =
      songChoosed_[app.currentIndex].querySelector(".btn-thumb-play");
    ...
    for (let i = 0; i < songChoosed.length; i++) {
      ...
      songChoosed[i].onmouseover = function (e) {
        for (let j = 0; j < songChoosed.length; j++) {
          const playThumbBtn__ =
            songChoosed[j].querySelector(".btn-thumb-play");
          const pauseThumbBtn__ =
            songChoosed[j].querySelector(".btn-thumb-pause");
          console.log(
            "active: " + songChoosed[i].classList.contains("active-song")
          );
      ...
      const watchList = songChoosed[i].querySelector(".watch-list");
